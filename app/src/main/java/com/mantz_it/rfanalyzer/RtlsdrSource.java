...
public class RtlsdrSource implements IQSourceInterface {
    ...
    public static final int RTL_TCP_COMMAND_SET_DIRECT_SAMPLING = 0x09;

    public static final int RTL_TCP_COMMAND_SET_BIAS_TEE = 0x0E;


    public final String[] COMMAND_NAME = {"invalid", "SET_FREQUENCY", "SET_SAMPLERATE", "SET_GAIN_MODE",
            "SET_GAIN", "SET_FREQ_CORR", "SET_IFGAIN", "SET_TEST_MODE", "SET_AGC_MODE", "SET_DIRECT_SAMPLING", "SET_BIAS_TEE"};
    ...
    private boolean directSampling = false;
    ...
    public void setManualGain(boolean enable) {
        if (isOpen()) {
            if (!commandThread.executeCommand(commandToByteArray(RTL_TCP_COMMAND_SET_GAIN_MODE, enable ? 0x01 : 0x00))) {
    ...
    public void setAutomaticGainControl(boolean enable) {
        if (isOpen()) {
            if (!commandThread.executeCommand(commandToByteArray(RTL_TCP_COMMAND_SET_AGC_MODE, enable ? 0x01 : 0x00))) {
    ...
    public void setRtlTcpCommandSetDirectSampling(boolean enable) {
        if (isOpen()) {
            if (!commandThread.executeCommand(commandToByteArray(RTL_TCP_COMMAND_SET_DIRECT_SAMPLING, enable ? 0x01 : 0x00))) {
                Log.e(LOGTAG, "setDirectSampling: failed.");
            }
        }
        this.directSampling = enable;
    }

    public void setRtlTcpCommandSetBiasTee(boolean enable) {
        if (isOpen()) {
            if (!commandThread.executeCommand(commandToByteArray(RTL_TCP_COMMAND_SET_BIAS_TEE, enable ? 0x01 : 0x00))) {
                Log.e(LOGTAG, "setBiasTee: failed.");
            }
        }
        this.directSampling = enable;
    }
